...
CD<-subset.data.frame(HistData,region=="cold_deserts")
CD<-CD[,-6]
head(CD)
...
#Vout<-variogram(Yresid~1,loc=~lat+long, width=.1,data=VarioData) ###calculate a variogram using the data. this will take a several minutes if the full dataset
#plot(Vout, main='CD')

phi<-4.5/3 ###kernal bandwidth---distance at which variogram stabalizes



plot(x[which(as.character(year)=="2015")],y[which(as.character(year)=="2015")], ylab="Lat", xlab="Long")###plot locations of all datapoints
knotloc<-expand.grid(seq(-122,-100,4.5),seq(37,50,4.5))
...
begin<-list(list('b'=c(b)),list('b'=c(b)),list('b'=c(b)))
...
CDfit = stan('Google Drive/research/NPP_Model/Regularization_scripts/GAUSstan.stan',iter=3000,init=begin, data=data,chains=3,refresh = 1,control = list(max_treedepth = 10, adapt_delta=.8), sample_file = 'GausCD.csv')
save.image("CDfitGaus.Rdata")  ###Save the whole workspace, not just model fit
